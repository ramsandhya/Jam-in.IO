...
function AudioSynthView() {
	...
	var fnPlayNote = function(instrument, note, octave) {
		console.log("the instrument is", instrument);
		src = __audioSynth.generate(instrument, note, octave, 2);
	...
	socket.on('fnPlayNote', function(data){
	  fnPlayNote(data.instrument, data.note, data.octave);
	});
